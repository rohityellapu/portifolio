    ...
    return (
        <div className={ `project lg:m-4 flex ${id % 2 === 0 ? 'lg:flex-row-reverse' : 'lg:flex-row'} flex-col-reverse lg:bg-inherit bg-[#041221] rounded-2xl m-2 p-2 justify-between` }>
            <div className="about flex items-center flex-col lg:w-1/2">
                ...
                <p>{ desc }</p>
                <div className="links flex justify-around w-full px-4 p-2">
            ...
            <div className={ `image ${isHovered ? 'hover' : ''} flex items-center lg:flex-row flex-col space-around rounded-lg h-56 lg:w-1/3 m-2 bg-cover bg-no-repeat` } style={ { backgroundImage: `url(${image})` } } onMouseEnter={ () => setisHovered(true) }
